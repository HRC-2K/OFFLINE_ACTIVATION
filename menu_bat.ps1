...
function Invoke-SubScript ($url, $fileName) {
    ...
    try {
        ...
        # Execute batch script and wait for completion
        Start-Process -FilePath "cmd.exe" -ArgumentList "/c `"$tempPath`"" -Wait -NoNewWindow
        
        # Clean up batch file after execution
        if (Test-Path $tempPath) {
            Remove-Item -Path $tempPath -Force -ErrorAction SilentlyContinue
            Write-Host "[CLEANUP]: Removed $fileName after execution." -ForegroundColor Gray
        }
    } catch {
...
function Install-App ($choice) {
    switch ($choice) {
        ...
        "11" { Invoke-SubScript "https://raw.githubusercontent.com/HRC-2K/OFFLINE_ACTIVATION/main/ALL_in_1.bat" "ALL_in_1.bat" }
        "12" { Invoke-SubScript "https://raw.githubusercontent.com/HRC-2K/OFFLINE_ACTIVATION/main/EA_Adapter_Offline_Method.bat" "EA_Adapter.bat" }
        "13" { Invoke-SubScript "https://raw.githubusercontent.com/HRC-2K/OFFLINE_ACTIVATION/main/Steam_Ubi_Epic_RStar.bat" "Steam_Ubi_Epic_RStar.bat" }
        "14" { 
