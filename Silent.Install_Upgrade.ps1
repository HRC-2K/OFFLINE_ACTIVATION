...
function Show-Menu {
    ...
    Write-Host " [1]  UltraViewer"[cite: 6]
    Write-Host " [2]  Cloudflare 1.1.1.1 WARP"[cite: 6]
    Write-Host " [3]  Firewall App Blocker (Extracts Only)"[cite: 6]
    Write-Host " [4]  Steam"[cite: 6]
    Write-Host " [5]  Ubisoft Connect"[cite: 6]
    Write-Host " [6]  Epic Games"[cite: 6]
    Write-Host " [7]  Rockstar Games (Manual Install)"[cite: 6]
    Write-Host " [8]  EA App"[cite: 6]
    Write-Host " [9]  TcNo Account Switcher"[cite: 6]
    Write-Host " [10] Bulk Crap Uninstaller"[cite: 6]
    Write-Host "------------------------------------------------------------------------- " -ForegroundColor DarkGray
    Write-Host " [11] Run All-in-1 Script" -ForegroundColor Green[cite: 6]
    Write-Host " [12] Run EA Adapter Offline Script" -ForegroundColor Green[cite: 6]
    Write-Host " [13] Run Steam/Ubi/Epic/Rockstar Script" -ForegroundColor Green[cite: 6]
    Write-Host " [14] Update Silent Install Master Script" -ForegroundColor Green[cite: 6]
    Write-Host "------------------------------------------------------------------------- " -ForegroundColor DarkGray
    Write-Host " [A]  Upgrade / Install ALL Items" -ForegroundColor Yellow[cite: 6]
    Write-Host " [X]  Exit" -ForegroundColor Red[cite: 6]
    Write-Host "=========================================================================" -ForegroundColor Cyan
    Write-Host " Files will be saved to: $InstallDir" -ForegroundColor Gray[cite: 6]
    Write-Host "=========================================================================" -ForegroundColor Cyan
}
...
function Process-App ($url, $fileName, $arguments, $processName) {
    ...
    Write-Host "Download finished. Processing..." -ForegroundColor Green
    if ($arguments -eq "ZIP") {
...
function Install-App ($choice) {
    switch ($choice) {
        ...
        "1"  { Process-App "https://www.ultraviewer.net/en/UltraViewer_setup_6.6_en.exe" "UltraViewer_setup_6.6_en.exe" "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART" "UltraViewer.exe" }[cite: 6]
        "2"  { Process-App "https://downloads.cloudflareclient.com/v1/download/windows/ga" "Cloudflare_1.1.1.1_Setup.msi" "/quiet /norestart ONBOARDING=false" "CloudflareWARP.exe" }[cite: 6]
        "3"  { Process-App "https://www.sordum.org/files/downloads.php?firewall-app-blocker" "FirewallAppBlocker.zip" "ZIP" "Fab.exe" }[cite: 6]
        "4"  { Process-App "https://cdn.akamai.steamstatic.com/client/installer/SteamSetup.exe" "SteamSetup.exe" "/S" "steam.exe" }[cite: 6]
        "5"  { Process-App "https://ubi.li/4vxt9" "UbisoftConnectInstaller.exe" "/S" "upc.exe" }[cite: 6]
        "6"  { Process-App "https://launcher-public-service-prod06.ol.epicgames.com/launcher/api/installer/download/EpicGamesLauncherInstaller.exe" "EpicGamesLauncherInstaller.exe" "/qn /norestart" "EpicGamesLauncher.exe" }[cite: 6]
        "7"  { Process-App "https://gamedownloads.rockstargames.com/public/installer/Rockstar-Games-Launcher.exe" "Rockstar-Games-Launcher.exe" "/s /v`"/qn`"" "Launcher.exe" }[cite: 6]
        "8"  { Process-App "https://origin-a.akamaihd.net/EA-Desktop-Client-Download/installer-releases/EAappInstaller.exe" "EAappInstaller.exe" "/q" "EADesktop.exe" }[cite: 6]
        "9"  { Process-App "https://github.com/TCNOco/TcNo-Acc-Switcher/releases/download/2025-11-20_03/TcNo.Account.Switcher.-.Installer_2025-11-20_03.exe" "TcNo.Account.Switcher.exe" "/S" "TcNo Account Switcher.exe" }[cite: 6]
        "10" { Process-App "https://github.com/BCUninstaller/Bulk-Crap-Uninstaller/releases/download/v6.2/BCUninstaller_6.2.0_setup.exe" "BCUninstaller_6.2.0_setup.exe" "/VERYSILENT /SUPPRESSMSGBOXES /NORESTART" "BCUninstaller.exe" }[cite: 6]
        "11" { 
...
do {
    Show-Menu
    $selection = Read-Host "`nEnter numbers separated by spaces (e.g., 0 1 4) or selection"[cite: 6]
...
} while ($selection.ToUpper() -ne "X")[cite: 6]